...
function gridX(scale, col, row) { return (scale * 0.5) + (col * scale * 0.75); }

function gridY(scale, col, row)
{
	if (col % 2) {
		return (scale * 0.870) + (row * scale * 0.865);
	} else {
		return (scale * 0.435) + (row * scale * 0.865);
	}
}

function gridCell(scale, col, row)
{
	let x = gridX(scale, col, row);
	let y = gridY(scale, col, row);

	return new Obj(polygon[6], "rgb(0, 0, 255)", scale, Math.PI / 6, x, y,
		       0, null);
}

function pieceObj(value, scale, col, row)
{
	return new Obj(
	 polygon[value], "rgb(0, 255, 0)", scale * 0.75, 0.0,
	 gridX(scale, col, row), gridY(scale, col, row), 1, (THIS) => {
		 if (THIS.hover) {
			 THIS.jit = 4;
		 } else {
			 THIS.jit = 1;
		 ...
		 if (THIS.grabbed) {
			 if (getDistance(THIS.anchorX, THIS.anchorY, mouseX,
					 mouseY)
			     > THIS.scale) {
				 THIS.x = THIS.anchorX
					  + THIS.scale
...
objList.push(gridCell(100, 0, 0));
objList.push(gridCell(100, 0, 1));
objList.push(gridCell(100, 0, 2));
objList.push(gridCell(100, 1, 0));
objList.push(gridCell(100, 1, 1));
objList.push(gridCell(100, 1, 2));
objList.push(gridCell(100, 2, 0));
objList.push(gridCell(100, 2, 1));
objList.push(gridCell(100, 2, 2));

objList.push(pieceObj(5, 100, 1, 0));
objList.push(pieceObj(3, 100, 1, 1));
